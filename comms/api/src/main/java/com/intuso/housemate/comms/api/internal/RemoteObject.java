    ...
    public static class TreeLoadInfo implements Message.Payload {
        ...
        public static TreeLoadInfo create(String[] path, String ending) {
            if(ending == null) {
                ...
                final RemoteObject.TreeLoadInfo root = new RemoteObject.TreeLoadInfo(path[0]);
                RemoteObject.TreeLoadInfo current = root;
                for(int i = 1; i < path.length; i++) {
                    RemoteObject.TreeLoadInfo child = new RemoteObject.TreeLoadInfo(path[i]);
                ...
                if(path == null || path.length == 0)
                    return new RemoteObject.TreeLoadInfo(ending);
                else {
                    final RemoteObject.TreeLoadInfo root = new RemoteObject.TreeLoadInfo(path[0]);
                    RemoteObject.TreeLoadInfo current = root;
                    for(int i = 1; i < path.length; i++) {
                    ...
                    RemoteObject.TreeLoadInfo child = new RemoteObject.TreeLoadInfo(ending);
