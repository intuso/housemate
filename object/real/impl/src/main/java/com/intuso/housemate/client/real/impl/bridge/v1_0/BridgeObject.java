...
public abstract class BridgeObject<
    ...
    public final void init(String versionName, String internalName, Connection connection) throws JMSException {
        ...
        sender = new com.intuso.housemate.client.real.impl.internal.JMSUtil.Sender(session, session.createProducer(session.createTopic(internalName)));
        receiver = new JMSUtil.Receiver<>(logger,
                session.createConsumer(session.createTopic(versionName)),
                versionDataClass,
                new JMSUtil.Receiver.Listener<VERSION_DATA>() {
                    @Override
    ...
    public interface Factory<OBJECT extends BridgeObject<?, ?, ?>> {
