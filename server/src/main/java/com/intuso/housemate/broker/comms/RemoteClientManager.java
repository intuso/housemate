...
public class RemoteClientManager {
    ...
    @Inject
    public RemoteClientManager(Log log, BrokerRealResources brokerRealResources, Storage storage, MainRouter mainRouter) {
        this.log = log;
    ...
    private BrokerRealUser getUserForSession(String sessionId) throws HousemateException {
        try {
            ...
            return brokerRealResources.getRoot().getUsers().get(userId);
    ...
    private BrokerRealUser authenticateUser(String username, String password) throws HousemateException {
        ...
        try {
            TypeInstanceMap details = storage.getValues(brokerRealResources.getRoot().getUsers().getPath(), userId);
